...
# Test client - using the correct syntax for current version
...
class TestDashboardEndpoints:
    ...
    @patch('app.routes.dashboard.get_current_user')
    @patch('app.routes.dashboard.get_db')
    def test_dashboard_page_authenticated(self, mock_db, mock_auth, sample_engagements, mock_user):
        """Test dashboard page access with authenticated user"""
        mock_auth.return_value = mock_user
        mock_db.return_value = AsyncMock()
        
        # Mock database operations
        with patch('app.routes.dashboard.get_user_engagements', return_value=sample_engagements), \
             patch('app.routes.dashboard.get_user_total_score', return_value=265), \
             patch('app.routes.dashboard.get_top_engagements', return_value=sample_engagements[:2]):
            
            response = client.get("/dashboard/")
            assert response.status_code == 200
            assert "Engagement Dashboard" in response.text
            assert "265" in response.text  # Total score
    
    def test_dashboard_page_unauthenticated(self):
        """Test dashboard page access without authentication"""
        response = client.get("/dashboard/")
    ...
    @patch('app.routes.dashboard.get_current_user')
    @patch('app.routes.dashboard.get_db')
    def test_engagements_api_authenticated(self, mock_db, mock_auth, sample_engagements, mock_user):
        """Test engagements API endpoint with authenticated user"""
        mock_auth.return_value = mock_user
        mock_db.return_value = AsyncMock()
        ...
        with patch('app.routes.dashboard.get_user_engagements', return_value=sample_engagements):
            ...
            response = client.get("/dashboard/api/engagements?sort_by=score&order=desc")
            assert response.status_code == 200
    ...
    def test_engagements_api_unauthenticated(self):
        """Test engagements API endpoint without authentication"""
        response = client.get("/dashboard/api/engagements")
    ...
    @patch('app.routes.dashboard.get_current_user')
    @patch('app.routes.dashboard.get_db')
    def test_stats_api_authenticated(self, mock_db, mock_auth, sample_engagements, mock_user):
        """Test stats API endpoint with authenticated user"""
        mock_auth.return_value = mock_user
        mock_db.return_value = AsyncMock()
        ...
        with patch('app.routes.dashboard.get_user_engagements', return_value=sample_engagements):
            ...
            assert data["total_score"] == 265
            assert data["average_score"] == 88.33
            assert data["top_score"] == 150
    
    def test_stats_api_unauthenticated(self):
        """Test stats API endpoint without authentication"""
        response = client.get("/dashboard/api/stats")
...
class TestDashboardFiltering:
    ...
    @patch('app.routes.dashboard.get_current_user')
    @patch('app.routes.dashboard.get_db')
    def test_score_range_filtering(self, mock_db, mock_auth, sample_engagements, mock_user):
        """Test filtering by score range"""
        mock_auth.return_value = mock_user
        mock_db.return_value = AsyncMock()
        
        # Mock database operations with score range filtering
        with patch('app.routes.dashboard.get_engagements_by_score_range', return_value=sample_engagements[:2]):
            
            response = client.get("/dashboard/api/engagements?min_score=50&max_score=200")
            assert response.status_code == 200
            
            data = response.json()
            assert len(data["tweets"]) == 2  # Should return 2 tweets in range
            assert data["filters"]["min_score"] == 50
            assert data["filters"]["max_score"] == 200
    
    @patch('app.routes.dashboard.get_current_user')
    @patch('app.routes.dashboard.get_db')
    def test_limit_filtering(self, mock_db, mock_auth, sample_engagements, mock_user):
        """Test limiting results"""
        mock_auth.return_value = mock_user
        mock_db.return_value = AsyncMock()
        
        # Mock database operations with limit
        with patch('app.routes.dashboard.get_user_engagements', return_value=sample_engagements[:2]):
            
            response = client.get("/dashboard/api/engagements?limit=2")
            assert response.status_code == 200
...
class TestDashboardErrorHandling:
    ...
    @patch('app.routes.dashboard.get_current_user')
    @patch('app.routes.dashboard.get_db')
    def test_database_error_handling(self, mock_db, mock_auth, mock_user):
        """Test handling of database errors"""
        mock_auth.return_value = mock_user
        mock_db.return_value = AsyncMock()
        
        # Mock database error
        with patch('app.routes.dashboard.get_user_engagements', side_effect=Exception("DB Error")):
            
            response = client.get("/dashboard/")
            assert response.status_code == 500
            assert response.json()["detail"] == "Error loading dashboard data"
    
    @patch('app.routes.dashboard.get_current_user')
    @patch('app.routes.dashboard.get_db')
    def test_api_error_handling(self, mock_db, mock_auth, mock_user):
        """Test handling of API errors"""
        mock_auth.return_value = mock_user
        mock_db.return_value = mock_user
        
        # Mock database error in API endpoint
        with patch('app.routes.dashboard.get_user_engagements', side_effect=Exception("DB Error")):
            
            response = client.get("/dashboard/api/engagements")
            assert response.status_code == 500
            assert response.json()["detail"] == "Error retrieving engagement data"
